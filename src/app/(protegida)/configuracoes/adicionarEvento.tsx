    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput style={styles.input} placeholder="Ex: Feijoada" value={states.nome} onChangeText={actions.setNome} />
        </View>

        <View style={styles.inputGroup}>
          <Text style={styles.label}><Text style={styles.required}>*</Text> Data</Text>
          <TouchableOpacity style={styles.selectInput} onPress={() => actions.setShowDatePicker(true)}>
            <Text style={[styles.selectText, states.dataSelecionada && { color: COLORS.textDark }]}>
              {states.dataSelecionada ? states.dataSelecionada.toLocaleDateString('pt-BR') : 'Selecione uma data'}
            </Text>
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}><Text style={styles.required}>*</Text> Horário</Text>
          <View style={styles.inputWithIcon}>
            <TextInput style={styles.inputFlex} placeholder="Ex: 14:30" keyboardType="numeric" maxLength={5} value={states.horario} onChangeText={actions.handleHorarioChange} />
        ...
        {/* --- SEÇÃO DESCRIÇÃO --- */}
        <View style={styles.inputGroup}>
          <Text style={styles.label}><Text style={styles.required}>*</Text> Descrição</Text>
          <TextInput
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput style={styles.input} placeholder="00000-000" keyboardType="numeric" value={states.cep} onChangeText={actions.handleCepChange} />
        </View>

        <View style={styles.inputGroup}>
          <Text style={styles.label}><Text style={styles.required}>*</Text> Rua</Text>
          <TextInput style={styles.input} placeholder="Ex: Rua das Mangueiras" value={states.rua} onChangeText={actions.setRua} />
        </View>

        <View style={styles.inputGroup}>
          <Text style={styles.label}><Text style={styles.required}>*</Text> Bairro</Text>
          <TextInput style={styles.input} placeholder="Ex: Rio Vermelho" value={states.bairro} onChangeText={actions.setBairro} />
        </View>
          ...
          <View style={styles.cityPickerContainer}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text style={[styles.cityItemText, states.cidade === item && { color: COLORS.orange, fontWeight: 'bold' }]}>
                    {item}
                  </Text>
                  {states.cidade === item && <Feather name="check" size={18} color={COLORS.orange} />}
                </TouchableOpacity>
              )}
            />
          </View>
...
const styles = StyleSheet.create({
  ...
  inputWithIcon: { height: 55, borderWidth: 1, borderColor: COLORS.border, borderRadius: 8, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.white },
  ...
  cityPickerContainer: { backgroundColor: COLORS.white, borderTopLeftRadius: 20, borderTopRightRadius: 20, height: '70%', padding: 20 },
  cityPickerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: '#EEE' },
  cityPickerTitle: { fontSize: 18, fontWeight: 'bold', color: COLORS.textDark },
  cityItem: { paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: '#F5F5F5', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
